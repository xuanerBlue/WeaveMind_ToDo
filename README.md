...
- **点条目打开详情**，可以写描述、列检查项。缩进的那一段属于上面那条待办，它整体跟着走。
  检查项只是父任务的进度刻度：不算待办、不进归档、不记完成日期，勾满了也不会让父任务自动完成——
  完成与否由你自己点。
...
- **新建的文件末尾会附一段格式说明**（上面这些规则的速查版），挂在 `## 格式说明` 标题下，
  软件不会动它，你想删随时删。已有的文件不会被自动改写——想补的话，⚙ 设置里点一下「写入文件末尾」。
